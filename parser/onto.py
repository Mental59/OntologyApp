...
from utils import remove_punctuation, stem_word
...
class Onto:
    ...
    def get_node_name(self, node_id):
        return self.node_dict[node_id]['original_name']

    def normalize_names(self):
        for node in self.nodes():
            node['original_name'] = node['name']
            words = nltk.word_tokenize(remove_punctuation(node['original_name']))
            words = [stem_word(word) for word in words]
            node['name'] = ' '.join(words)

    def get_normalized_names(self):
        return [node['name'] for node in self.nodes()]

    def get_source_names(self):
        return [node['original_name'] for node in self.nodes()]
    ...
    def get_sub_onto(self, node_ids: list):
        data = deepcopy(self.data)
        data['history'] = []
        if len(node_ids) == 1:
            data['nodes'] = [self.node_dict[node_ids[0]]]
            data['relations'] = []
        else:
            subgraph = self.__create_subgraph(node_ids)
            data['nodes'] = [node for node in data['nodes'] if node['id'] in subgraph.nodes]
            data['relations'] = [
                link for link in data['relations']
                if (link['source_node_id'], link['destination_node_id']) in subgraph.edges
                or (link['destination_node_id'], link['source_node_id']) in subgraph.edges
            ]
        self.__remove_additional_data(data)
        return data

    def __remove_additional_data(self, data):
        for node in data['nodes']:
            node['name'] = node['original_name']
            del node['original_name']

    def __create_subgraph(self, node_ids):
        undirected_graph = nx.to_undirected(self.digraph)
        res_nodes = set()
        for node_id1 in node_ids:
            paths = [
                nx.shortest_path(undirected_graph, node_id1, node_id2) for node_id2 in node_ids if node_id1 != node_id2
            ]
            for path in paths:
                res_nodes.update(path)
